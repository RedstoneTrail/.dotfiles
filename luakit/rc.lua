...
webview.add_signal("init", function(view)
    view:add_signal("navigation-request", function(v, uri)
        -- open pdfs in system viewer
        local start, finish = string.find(uri, ".pdf")
        if (start ~= nil) then
            local handle = io.popen("mktemp --suffix=.pdf --tmpdir=/tmp luakit-file.XXXXX")
            local tmpfile = handle:read("*a")
            -- trim newline
            tmpfile = string.sub(tmpfile, 0, -2)

            luakit.spawn(string.format("wget -O %q %q", tmpfile, uri))
            luakit.spawn(string.format("sh -c 'xdg-open %q; rm %q'", tmpfile, tmpfile))
            return false
        end
    end)
end)
...
-- newtab_chome.new_tab_file = settings.window.home_page
-- w:notify(newtab_chrome.new_tab_file)

local engines = settings.window.search_engines

engines.br = "https://search.brave.com/search?q=%s"

engines.np = "https://search.nixos.org/packages?query=%s"
engines.no = "https://search.nixos.org/options?query=%s"

engines.zd = "https://ziglang.org/documentation/master/"
engines.zs = "https://ziglang.org/documentation/master/std/#%s"

engines.cl = "https://classroom.google.com/u/1/"

engines.rd = "https://docs.rs/%s"
engines.default = engines.br

settings.tablist.always_visible = false

settings.webview.zoom_level = 133

os.execute("notify-send 'correctly sourced'")
