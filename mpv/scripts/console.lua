...
local function should_scale()
	return opts.scale_with_window == "yes"
		or (opts.scale_with_window == "auto" and mp.get_property_native("osd-scale-by-window"))
end
...
local function calculate_max_lines()
	if terminal_output() then
		...
		return mp.get_property_native("term-size/h", 24)
			- 2
	...
	return math.floor(
		(
			select(2, get_scaled_osd_dimensions()) * (1 - global_margins.t - global_margins.b)
			- get_margin_y()
			- (selectable_items and opts.padding * 2 or 0)
		)
				/ get_line_height()
...
local function calculate_max_item_width()
	...
	width_overlay.data = "{\\fs"
		.. opts.font_size
		.. (font and "\\fn" .. font or "")
		.. "\\q2}"
		.. ass_escape(longest_item)
	local result = width_overlay:update()
	max_item_width = math.min(result.x1 - result.x0, osd_w - get_margin_x() * 2 - opts.padding * 2)
end

local function should_highlight_completion(i)
	return i == selected_completion_index or (i == 1 and selected_completion_index == 0 and autoselect_completion)
end
...
local function get_selected_ass()
	...
	local outline_color, outline_alpha = mpv_color_to_ass(mp.get_property("osd-selected-outline-color"))
	return "{\\b1\\1c&H"
		.. color
		.. "&\\1a&H"
		.. alpha
...
local function format_grid(list, width_max, rows_max)
	...
	local spacing = column_count > 1 and ass_escape(string.format("%" .. spaces .. "s", " ")) or ""
	...
	for row = 1, row_count do
		...
		for column = 1, column_count do
			...
			local format_string = column == column_count and "%s" or "%-" .. math.min(column_widths[column], 99) .. "s"
			columns[column] = ass_escape(string.format(format_string, list[i]))

			if should_highlight_completion(i) then
				columns[column] = get_selected_ass() .. columns[column] .. "{\\b\\1a&\\3a&}" .. styles.completion
			end
		end
...
local function get_matches_to_print(terminal)
	...
	local escape = terminal and function(str)
		return str
	end or ass_escape
	local highlight = terminal and terminal_styles.matched_position
		or "{\\1c&H" .. color_option_to_ass(opts.match_color) .. "}"
	...
	local last_match_to_print = math.min(first_match_to_print + max_lines - 1, #matches)
	...
	for i = first_match_to_print, last_match_to_print do
		...
		if terminal then
			...
			if i == focused_match then
				if searching_history and mp.get_property("osd-border-style") == "outline-and-shadow" then
...
local function update_overlay(data, res_x, res_y, z)
	if overlay.data == data and overlay.res_x == res_x and overlay.res_y == res_y and overlay.z == z then
		return
	end
...
local function print_to_terminal()
	...
	if selectable_items then
		...
		for i, completion in ipairs(completion_buffer) do
			if should_highlight_completion(i) then
				log = log .. terminal_styles.selected_completion .. completion .. "\027[0m"
			else
				log = log .. completion
			end
...
local function render()
	...
	if selectable_items and not searching_history then
		...
		y = osd_h * (global_margins.t + (1 - global_margins.t - global_margins.b) / 2)
	...
	local cglyph = "{\\r\\blur0"
		.. (get_property_cached("focused") == false and "\\alpha&HFF&" or "\\3a&H" .. alpha .. "&")
		.. "\\3c&H"
		.. color
		.. "&"
	...
	for i = #log_buffer - math.min(max_lines, #log_buffer) + 1, #log_buffer do
		log_ass = log_ass .. style .. log_buffer[i].style .. ass_escape(log_buffer[i].text) .. "\\N"
	end
	...
	if completion_buffer[1] and not selectable_items then
		...
		local width_max =
			math.floor((osd_w - x - mp.get_property_native("osd-margin-x") * 2) / opts.font_size * get_font_hw_ratio())
	...
	-- Background
	if selectable_items and (not searching_history or border_style == "background-box") then
		style = style .. "{\\bord0\\blur0\\4a&Hff&}"
		local back_color, back_alpha = mpv_color_to_ass(
			mp.get_property(border_style == "background-box" and "osd-back-color" or "osd-outline-color")
		)
		...
		ass:round_rect_cw(
			...
			(1.5 + math.min(#matches, max_lines)) * line_height + opts.padding * (alignment == 7 and 1 or 2),
			opts.corner_radius,
			opts.corner_radius
		)
		ass:draw_stop()
	...
	for i, item in ipairs(items) do
		local item_y = alignment == 7 and y + (1 + i) * line_height or y - (1.5 + #items - i) * line_height

		if
			(
				first_match_to_print - 1 + i == focused_match
				or matches[first_match_to_print - 1 + i].index == default_item
			) and (not searching_history or border_style == "background-box")
		then
			ass:new_event()
			ass:an(4)
			ass:pos(x, item_y)
			ass:append("{\\blur0\\bord0\\4aH&ff&\\1c&H" .. color_option_to_ass(opts.focused_back_color) .. "&}")
		...
		item_positions[#item_positions + 1] = { item_y - line_height / 2, item_y + line_height / 2 }
	end
	...
	if selectable_items and #matches > max_lines then
		...
		local bar_y = alignment == 7 and y + 1.5 * line_height + start_percentage * max_height
...
local function submit()
	...
	if selectable_items then
		...
		mp.commandv("script-message-to", input_caller, "input-event", "submit", utils.format_json({ line }))
...
local function determine_hovered_item()
	...
	local item_x0 = (searching_history and get_margin_x() or (osd_w - max_item_width) / 2) - opts.padding
...
local function move_history(amount, is_wheel)
	...
	if is_wheel then
		...
		if
			(amount > 0 and focused_match == first_match_to_print and first_match_to_print - 1 + max_lines < #matches)
			or (amount < 0 and focused_match == first_match_to_print - 1 + max_lines and first_match_to_print > 1)
		then
			focused_match = focused_match + amount
		end

		if
			amount > 0 and first_match_to_print < #matches - max_lines + 1
			or amount < 0 and first_match_to_print > 1
		then
			-- math.min and math.max would only be needed with amounts other than
			-- 1 and -1.
			first_match_to_print = math.min(math.max(first_match_to_print + amount, 1), #matches - max_lines + 1)
		end
...
local function strip_common_characters(str, prefix)
	return str:sub(1 + math.max(common_prefix_length(prefix, str), max_overlap_length(prefix, str)))
end
...
cycle_through_completions = function(backwards)
	...
	local before_cur = line:sub(1, completion_pos - 1)
		.. completion_buffer[selected_completion_index]
		.. completion_append
	line = before_cur
		.. strip_common_characters(line:sub(cursor), completion_buffer[selected_completion_index] .. completion_append)
	cursor = before_cur:len() + 1
	render()
end
...
complete = function()
	...
	mp.commandv(
		...
		"input-event",
		"complete",
		utils.format_json({ line:sub(1, cursor - 1) })
	)
	render()
end
...
local function get_bindings()
	local bindings = {
		...
		{ "ctrl+i", cycle_through_completions },
		{
			"shift+tab",
			function()
				cycle_through_completions(true)
			end,
		},
		{ "ctrl+a", go_home },
		{ "home", go_home },
...
set_active = function(active)
	...
	elseif searching_history then
		...
		mp.commandv("script-message-to", input_caller, "input-event", "closed", utils.format_json({ line, cursor }))
		collectgarbage()
	end
	render()
end
...
mp.register_script_message("log", function(message)
	...
	log_buffer[#log_buffer + 1] = {
		...
		terminal_style = message.error and terminal_styles.error or message.terminal_style or "",
	}
...
mp.register_script_message("complete", function(list, start_pos, append)
	...
	for i, match in ipairs(fuzzy_find(line:sub(completion_pos, cursor - 1), completions)) do
